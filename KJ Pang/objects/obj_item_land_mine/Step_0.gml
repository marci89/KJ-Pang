...
if (place_meeting(x, y, obj_player_one)) {
	...
	if(!isPickedUp && !obj_player_one.isDead) {
		obj_player_one.landMineAmmo  += ammoCapacity;
	    obj_player_one.weapon = weaponType.LandMine;
...
if (place_meeting(x, y, obj_player_two)) {
	...
	if(!isPickedUp && !obj_player_two.isDead) {
		obj_player_two.landMineAmmo  += ammoCapacity;
	    obj_player_two.weapon = weaponType.LandMine;
		PlaySound(snd_bomb_pickup, false);
		isPickedUp = true;
	}
}
