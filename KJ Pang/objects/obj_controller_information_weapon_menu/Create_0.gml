...
//texts
titleText = GetLocalizedText("weapons"); 
...
function GetSelectedItemInformationType() {
		...
		var totalItems = instance_number(obj_information_weapons_item_button);
		
			for (var i = 0; i < totalItems; i++) {	
			    var item = instance_find(obj_information_weapons_item_button, i);
				  if (item != noone) {
...
for (var i = 0 ; i < itemNumber; i++) {
    ...
    var result = instance_create_layer(startX + distance, buttonY,"TopLayer", obj_information_weapons_item_button);
	result.buttonId = i;
	result.Init();
	currentXpos += 1;
 ...
 //back button
 var backButton = instance_create_layer(startX + 150, buttonY,"TopLayer", obj_information_weapons_menu_back_button);
