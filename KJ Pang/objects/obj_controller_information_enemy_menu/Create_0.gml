...
global.isEnemyInformationMode = true; // helper for enemy information
//info type
infoType = noone;

//texts
titleText = GetLocalizedText("enemies"); 
...
function GetSelectedItemInformationType() {
		...
		var totalItems = instance_number(obj_information_enemies_item_button);
		
			for (var i = 0; i < totalItems; i++) {	
			    var item = instance_find(obj_information_enemies_item_button, i);
				  if (item != noone) {
...
function RefreshInfoPanel() {
	...
	var infoPanelObj = obj_information_enemy_panel;
...
for (var i = 0 ; i < itemNumber; i++) {
    ...
    var result = instance_create_layer(startX + distance, buttonY,"TopLayer", obj_information_enemies_item_button);
	result.buttonId = i;
	result.Init();
	currentXpos += 1;
 ...
 //back button
 var backButton = instance_create_layer(startX + 150, buttonY,"TopLayer", obj_information_enemies_menu_back_button);
...
#region Create info panel
    ...
    var infoPanelObj = instance_create_layer(infoPanelStartX, infoPanelStartY,"Instances", obj_information_enemy_panel);
	infoPanelObj.Init(infoType ?? noone);
